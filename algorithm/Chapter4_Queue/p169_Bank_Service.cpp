...
void bank::business_start() {
	...
	for (int i = 0; i < business_hours; i++)
	{
		...
		//==========은행원 상태 확인==========
		int remaining_time = worker.get_time();		//은행원의 남은 업무처리 시간
		if (remaining_time <= 0)
			cout << "은행원 : 응대준비 완료" << endl;

		//==========고객방문 확인==========
		int rand = random(gen);
		//분당 50프로의 확률 : 0, 1, 2, 3, 4
		if (rand < 5) {								//방문했을경우
		...
		if (remaining_time <= 0) {
			if (!queue.is_empty()) {				//응대준비가 되었고 대기열이 있을경우
				...
				cout << "은행원 : " << id << "번 고객님 업무처리 시작!. 대기시간 : " << waiting_time << "분 소요 / 남은시간 : " << service_time << "분" << endl;
				worker.minus_time();
			}
		}
		else {
			int id = worker.get_customer().get_id();
			cout << "은행원 : " << id << "번 고객님 업무처리중. 남은시간 : " << remaining_time << "분" << endl;
			worker.minus_time();
		}
		queue.print_queue();
	}
