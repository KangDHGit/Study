...
#pragma endregion
	...
	static ListNode* insert(ListNode* head, ListNode* pre, element data) {
	...
	//도전과제
	static element get_entry(ListNode* head, int index) {
		if (head == NULL) { cout << "리스트가 NULL 입니다." << endl; return element{ -1 }; }
		ListNode* resultNode = head;
		for (int i = 0; i < index; i++)
		{
			if (resultNode == NULL) { cout << "잘못된 인덱스 값입니다." << endl; return element{ -1 }; }
...
int main()
{
	...
	element data = ListNode::get_entry(head, 3);
